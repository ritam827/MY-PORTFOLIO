...
function App() {
  ...
  const links = ['home', 'about', 'education', 'skills', 'projects', 'experience', 'hackathons', 'certifications', 'contact']
  ...
  return <div className="site-shell">
    ...
    <main><DataNetwork /><div className="enhancement-dock"><AnalyticsDashboard /><TerminalCard /></div>
      ...
      <section id="education" className="section container"><SectionHeading eyebrow="02 / Education" title="Learning the craft, one layer at a time." /><div className="timeline">{education.map((item, index) => <motion.article className="timeline-item" key={item.title} variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true }} transition={{ delay: index * 0.1 }}><div className="timeline-dot">0{index + 1}</div><div className="timeline-content"><span className="period">{item.period}</span><h3>{item.title}</h3><p>{item.school}</p><strong>{item.detail}</strong></div></motion.article>)}</div></section>
      ...
      <section id="certifications" className="section container"><SectionHeading eyebrow="07 / Certifications" title="Proof of consistent curiosity." /><div className="cert-grid">{certifications.map(([title, org], index) => <motion.article className="cert-card" key={title} variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true }}><div className="cert-icon"><GraduationCap size={19} /></div><span className="cert-index">0{index + 1}</span><h3>{title}</h3><p>{org}</p><button className="text-link muted" disabled>View certificate <ArrowUpRight size={14} /></button></motion.article>)}</div><p className="note">Certificate files can be linked automatically from <code>public/certificates/</code> when added.</p></section>
      ...
      <section className="stack-strip"><div className="container"><div className="stack-label">Working across the stack <span>↗</span></div><div className="marquee"><div className="marquee-track">{[...techStack, ...techStack].map((tech, index) => <span key={`${tech}-${index}`}>{tech} <b>·</b></span>)}</div></div></div></section>
